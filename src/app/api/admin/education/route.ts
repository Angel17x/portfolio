...
import { EducationSchema } from "@/lib/validations/portfolio-schemas"
...
export async function GET() {
  try {
    ...
    console.error("Error obteniendo education:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const snapshot = await db.collection("education").orderBy("order", "desc").limit(1).get()
    ...
    const docRef = await db.collection("education").add(dataWithOrder)

    // Registrar acción
    await logCRUDAction(request, "create", "education", docRef.id, dataWithOrder)
    ...
    console.error("Error creando education:", error)
    return NextResponse.json({ error: "Error interno del servidor" }, { status: 500 })
  }
}
