...
export async function PUT(
  ...
  try {
    ...
    await db.collection("education").doc(id).set(validatedData)

    // Registrar acción
    await logCRUDAction(request, "update", "education", id, validatedData)
    ...
    console.error("Error actualizando education:", error)
    return NextResponse.json({ error: "Error interno del servidor" }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    // Registrar acción
    await logCRUDAction(request, "delete", "education", id)
