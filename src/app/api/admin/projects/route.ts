...
export async function POST(request: NextRequest) {
  try {
    ...
    const snapshot = await db.collection("projects").orderBy("order", "desc").limit(1).get()
    ...
    const docRef = await db.collection("projects").add(dataWithOrder)

    // Registrar acción
    await logCRUDAction(request, "create", "projects", docRef.id, dataWithOrder)
    ...
    console.error("Error creando project:", error)
    return NextResponse.json({ error: "Error interno del servidor" }, { status: 500 })
  }
}
